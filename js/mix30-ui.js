/* MIX43: collapsible Today journey map. Embedded map is OSM/Leaflet with local schematic fallback. */
(function(){
  'use strict';
  ...
  function render(){const box=document.getElementById('todayJourneyMap');if(!box||!window.GSPA_RouteMap)return;const d=operationalDay(),pts=ROUTE_POINTS[d.date]||[];mapDate=d.date;setState(navigator.onLine===false?'오프라인 동선도':'동선 지도를 불러오는 중…');window.GSPA_RouteMap.render(box,pts,{label:d.title}).then(ok=>setState(ok?'OpenStreetMap · 번호 순서':'오프라인 동선도 · Google 지도 버튼 사용')).catch(()=>setState('오프라인 동선도 · Google 지도 버튼 사용'))}
