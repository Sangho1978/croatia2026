/* MIX44: collapsible Today journey map. Google Maps online with local schematic fallback. */
(function(){
  'use strict';
  ...
  function render(){const box=document.getElementById('todayJourneyMap');if(!box||!window.GSPA_RouteMap)return;const d=operationalDay(),pts=ROUTE_POINTS[d.date]||[];mapDate=d.date;setState(navigator.onLine===false?'오프라인 동선도':'Google Maps 불러오는 중…');window.GSPA_RouteMap.render(box,pts,{label:d.title}).then(ok=>setState(ok?'Google Maps · 번호 순서':'Google Maps 연결 필요 · 아래 전체 동선 버튼 사용')).catch(()=>setState('Google Maps 연결 필요 · 아래 전체 동선 버튼 사용'))}
