...
(function(){
  ...
  function renderRecords(){
    ...
    const activeCount=Object.values(records).filter(activeRecord).length,deletedCount=Object.values(records).filter(r=>r?.deleted).length;const info=document.getElementById('expenseListInfo');if(info)info.textContent=(storageReady?'사용일 기준 공유기록 '+activeCount+'건 중 '+items.length+'건 표시':'연결 후 공유 기록을 불러옵니다.')+' · EUR/KRW 별도 합산'+(deletedCount?' · 삭제 '+deletedCount+'건 정산 제외':'');
  ...
  function exportSettlementExcel(){
    ...
    for(const [id,r] of entries){const c=recordCreated(r),u=recordUpdated(r);expenseRows.push([usageDateOf(r),r.time||'',r.merchant||'',r.title||'',r.category||'',r.currency,r.currency==='EUR'?r.amountMinor/100:r.amountMinor,r.participantCount,Object.values(r.participants||{}).map(x=>x.name).join(', '),r.payerName||'',r.fxRate||'',r.fxRateDate||'',r.estimatedKrw||'',r.receiptSummary?.count||0,r.createdByName||'',c.croatia,c.korea,r.updatedByName||'',u.croatia,u.korea,id])}
