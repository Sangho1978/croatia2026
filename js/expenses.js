...
(function(){
  ...
  async function save(){
    ...
    try{requireStaff();const data=validate();if(!storageReady)throw Error('공유 저장 연결을 먼저 확인해 주세요. 입력은 기기 초안으로 보관합니다.');
      ...
      const r={...data,id,createdBy:original?.createdBy||uid,createdByName:original?.createdByName||name,createdAt:original?.createdAt||{'.sv':'timestamp'},updatedBy:uid,updatedByName:name,updatedAt:{'.sv':'timestamp'},revision:(original?.revision||0)+1,...(receiptSummary?{receiptSummary}:{})};
