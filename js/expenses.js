...
(function(){
  const FOREIGN=(window.TRIP_CURRENCY||window.GSPA_TRIP?.currency||'EUR'); const FOREIGN_SYMBOL=FOREIGN==='EUR'?'€':FOREIGN==='TRY'?'₺':FOREIGN+' ';
  ...
  let expenseTab='entry',ledgerDay='',ledgerNotice='',fxQuote=null,fxBusy=false,fxSeq=0,deleteBusy=false;
  const TRIP_DATES=(Array.isArray(window.days)?window.days:typeof days!=='undefined'?days:[]).map(d=>d.date);
  ...
  const draftKey=()=> (window.GSPA_TRIP_STORAGE_PREFIX||'cro')+'.expense.draft.v2.'+(currentUser?.name||'');
  const oldDraftKey=()=> (window.GSPA_TRIP_STORAGE_PREFIX||'cro')+'.expense.draft.v1.'+(currentUser?.name||'');
  const status=(text,kind='')=>Integration.feedback(document.getElementById('expenseFeedback'),text,kind);
  function money(minor,currency){return currency==='KRW'?'₩'+minor.toLocaleString('ko-KR'):(currency==='EUR'?'€':currency==='TRY'?'₺':currency+' ')+(minor/100).toLocaleString('ko-KR',{minimumFractionDigits:2,maximumFractionDigits:2})}
  function currentPlace(){
    ...
    if(Number.isFinite(lat)&&Number.isFinite(lng)){
      ...
      if(lat>=35.5&&lat<=42.5&&lng>=25.0&&lng<=45.0)return {label:'튀르키예',zone:'Europe/Istanbul'};
    }
    const zone=Intl.DateTimeFormat().resolvedOptions().timeZone||(window.GSPA_TRIP_TIMEZONE||'Europe/Zagreb');
    return {label:zone==='Asia/Seoul'?'한국':zone==='Europe/Rome'?'이탈리아':zone==='Europe/Istanbul'?'튀르키예':zone==='Europe/Zagreb'?'크로아티아':'현재 위치',zone};
  }
  ...
  function usageDateOf(r){return validDate(r?.date)?r.date:''}
  function activeRecord(r){return !!(r&&r.id&&!r.deleted&&usageDateOf(r))}
  function shortDate(d){if(!validDate(d))return '-';const [y,m,day]=d.split('-').map(Number);return `${m}월 ${day}일`}
  function clockPair(clock,ts){const d=dualTime(typeof ts==='number'?ts:(clock?.epoch||0));return {croatia:clock?.croatia?String(clock.croatia).slice(5,16):d.croatiaShort,korea:clock?.korea?String(clock.korea).slice(5,16):d.koreaShort}}
  function recordCreated(r){return clockPair(r?.createdClock,r?.createdAt)}
  function recordUpdated(r){return clockPair(r?.updatedClock,r?.updatedAt)}
  ...
  function requireStaff(){if(!Integration.canManageFinance())throw Error('공동경비는 '+(window.GSPA_TRIP?.financeManagers||['한상호','이상미']).join('·')+'만 사용합니다.')}
  function weekday(date){try{return new Intl.DateTimeFormat('ko-KR',{weekday:'short',timeZone:(window.GSPA_TRIP_TIMEZONE||'Europe/Zagreb')}).format(new Date(date+'T12:00:00+02:00'))}catch(_){return''}}
    ...
    <div class="expense-layout-v2">
      <section class="expense-pane expense-pane-entry" data-expense-pane="entry" ${manager?'':'hidden'}>
        <div class="finance-panel"><h3 id="expenseFormTitle">공동경비 등록</h3><form id="expenseForm" class="finance-form">
          ...
          <div class="finance-row"><label class="field">통화<select id="exCurrency"><option value="${FOREIGN}">${FOREIGN} · 현지통화</option><option value="KRW">KRW · 원</option></select></label><label class="field">전체 금액<input id="exAmount" inputmode="decimal" placeholder="0.00" required /></label></div><div id="expenseFxEstimate" class="expense-fx-estimate">${FOREIGN==='EUR'?'유로 금액을 입력하면 하나은행 매매기준율 기준 예상 원화를 계산합니다.':'현지통화와 원화를 별도 합산합니다.'}</div>
          <label class="field">결제자<input id="exPayer" maxlength="60" placeholder="실제 결제한 사람 또는 공용카드" /></label>
          <h4 style="margin:5px 0">② 참석명단</h4><div class="participant-quick"><button type="button" id="importAttendance">✓ 최근 출석명단 불러오기</button><small>가장 최근 출석체크 완료자를 참여자로 한 번에 선택합니다.</small></div><div class="expense-mode" role="group" aria-label="경비 참여 범위"><button type="button" data-expense-mode="all">전체 참석 · ${APP_USERS.length}명</button><button type="button" data-expense-mode="selected">개별 선택</button></div>
          <div class="part-count" aria-live="polite"><span><b id="exParticipantCount">${APP_USERS.length}</b>명 참여</span><small id="exSelectionNote">교수님 포함 전체</small></div>
  ...
  function renderBase(){
    ...
    if(document.getElementById('ledgerExportXlsx'))document.getElementById('ledgerExportXlsx').onclick=exportSettlementExcel;
  ...
  function renderFxEstimate(){
    ...
    if(!fxQuote||!Number.isFinite(amt)||amt<=0){box.className='expense-fx-estimate';box.textContent=FOREIGN==='EUR'?'유로 금액을 입력하면 하나은행 매매기준율 기준 예상 원화를 계산합니다.':'현지통화와 원화를 별도 합산합니다.';return}
  ...
  async function load(){
    ...
    try{const r=await Integration.api('expenses/'+TRIP_CODE,{headers:{'X-Firebase-ETag':'true'}});if(epoch!==fetchEpoch||currentUser?.name!==name)return;records=r.data||{};storageReady=true;renderRecords();accessMessage(Integration.canManageFinance()?'등록·수정 가능 · 장부 연결됨':APP_USERS.length+'명 공용 장부 · 조회 전용',true)}
  ...
  function renderRecords(){
    if(!document.getElementById('expenseRecords'))return;const items=visibleItems(),sums={[FOREIGN]:0,KRW:0},daily={};let estKrw=0;items.forEach(([,r])=>{const date=usageDateOf(r);if(r.currency in sums&&Number.isFinite(r.amountMinor)){sums[r.currency]+=r.amountMinor;(daily[date]??={[FOREIGN]:0,KRW:0,n:0,estKrw:0})[r.currency]+=r.amountMinor;daily[date].n++}if(Number.isFinite(r.estimatedKrw)){estKrw+=r.estimatedKrw;(daily[date]??={[FOREIGN]:0,KRW:0,n:0,estKrw:0}).estKrw+=r.estimatedKrw}});renderDayChips();
    const notice=document.getElementById('ledgerNotice');if(notice){notice.hidden=!ledgerNotice;notice.textContent=ledgerNotice}const hero=document.getElementById('ledgerHero');if(hero)hero.innerHTML=`<div class="ledger-kpi"><small>${ledgerDay==='all'?'조회 기록':'선택 사용일'}</small><b>${items.length}건</b></div><div class="ledger-kpi"><small>${FOREIGN} 합계</small><b>${money(sums[FOREIGN],FOREIGN)}</b></div><div class="ledger-kpi"><small>KRW 합계</small><b>${money(sums.KRW,'KRW')}</b></div><div class="ledger-kpi"><small>원화 환산 추정</small><b>${estKrw?'≈ '+money(estKrw,'KRW'):'-'}</b></div>`;
    const activeCount=Object.values(records).filter(activeRecord).length,deletedCount=Object.values(records).filter(r=>r?.deleted).length;const info=document.getElementById('expenseListInfo');if(info)info.textContent=(storageReady?'사용일 기준 공유기록 '+activeCount+'건 중 '+items.length+'건 표시':'연결 후 공유 기록을 불러옵니다.')+' · '+FOREIGN+'/KRW 별도 합산'+(deletedCount?' · 삭제 '+deletedCount+'건 정산 제외':'');
    const grouped={};items.forEach(x=>(grouped[usageDateOf(x[1])]??=[]).push(x));
    document.getElementById('expenseRecords').innerHTML=Object.keys(grouped).length?Object.entries(grouped).sort(([a],[b])=>b.localeCompare(a)).map(([date,rows])=>{
      const d=daily[date]||{[FOREIGN]:0,KRW:0,n:0,estKrw:0},tot=[d[FOREIGN]?money(d[FOREIGN],FOREIGN):'',d.KRW?money(d.KRW,'KRW'):''].filter(Boolean).join(' / ');
      return `<section class="ledger-day-group"><div class="ledger-day-head"><h4>${shortDate(date)} (${weekday(date)}) · 사용내역</h4><span>${d.n}건 · ${tot||'합계 없음'}</span></div>${rows.map(([id,r])=>entryMarkup(id,r)).join('')}</section>`;
    }).join(''):'<div class="ledger-empty">이 사용일에 저장된 공동경비가 없습니다.</div>';
  }
  function entryMarkup(id,r){
    const names=Object.values(r.participants||{}).map(u=>u.name),photos=r.receiptSummary?.count||0,merchant=r.merchant||'',useDate=usageDateOf(r),created=recordCreated(r),updated=recordUpdated(r),wasEdited=(Number(r.revision||1)>1)||((typeof r.createdAt==='number'&&typeof r.updatedAt==='number')&&Math.abs(r.updatedAt-r.createdAt)>2000);
    const saved=`저장 현지 ${created.croatia} · 한국 ${created.korea}`;const modified=wasEdited?`<div class="expense-audit modified">수정 현지 ${updated.croatia} · 한국 ${updated.korea} · ${escape(r.updatedByName||'')}</div>`:'';
    return `<article class="expense-entry" data-expense-id="${escape(id)}"><div class="expense-entry-head"><div>${merchant?`<div class="expense-merchant">${escape(merchant)}</div>`:''}<h4 class="expense-purpose">${escape(r.title)}</h4></div><b>${money(r.amountMinor,r.currency)}</b></div><div class="expense-use-date"><strong>사용일 ${shortDate(useDate)}</strong>${r.time?`<span>사용시각 ${escape(r.time)}</span>`:''}</div><div class="ledger-primary"><strong>${r.participantCount}명 참여</strong><span>${escape(r.category)}</span></div>${r.currency==='EUR'&&Number.isFinite(r.estimatedKrw)?`<div class="expense-krw-line">≈ ${money(r.estimatedKrw,'KRW')} · 1€=₩${Number(r.fxRate||0).toLocaleString('ko-KR',{maximumFractionDigits:2})} · ${escape(r.fxRateDate||'')} ${r.fxExact===false?'추정':''}</div>`:''}<div class="meta">결제 ${escape(r.payerName||'미지정')} · 등록 ${escape(r.createdByName||r.updatedByName||'')}</div><div class="expense-audit">${saved}</div>${modified}<details><summary>참석명단 · ${r.participantCount}명</summary><div>${names.map(escape).join(' · ')}${r.memo?'<p>'+escape(r.memo)+'</p>':''}</div></details><div class="ledger-tools">${Integration.canManageFinance()?`<button type="button" class="edit-expense" data-edit-expense="${escape(id)}">수정</button><button type="button" class="delete-expense" data-delete-expense="${escape(id)}">삭제</button>`:''}${photos?`<button type="button" data-view-receipts="${escape(id)}">영수증 ${photos}장 보기</button>`:'<span class="quiet">영수증 없음</span>'}</div><div class="receipt-gallery" data-receipt-gallery="${escape(id)}" hidden></div></article>`;
  }
  function activeEntriesSorted(){return Object.entries(records).filter(([,r])=>activeRecord(r)).sort((a,b)=>usageDateOf(a[1]).localeCompare(usageDateOf(b[1]))||(a[1].createdAt||0)-(b[1].createdAt||0))}
  function exportLedger(){
    if(!Integration.canManageFinance()||!storageReady)return;const rows=[['사용일','사용시각','사용처','사용목적','분류','통화','금액','참여인원','참여자','결제자','메모','영수증 장수','하나환율','환율기준일','원화환산추정','등록자','저장시각(크로아티아)','저장시각(한국)','수정자','수정시각(크로아티아)','수정시각(한국)'],...activeEntriesSorted().map(([,r])=>{const c=recordCreated(r),u=recordUpdated(r);return [usageDateOf(r),r.time||'',r.merchant||'',r.title,r.category,r.currency,r.currency!=='KRW'?(r.amountMinor/100).toFixed(2):r.amountMinor,r.participantCount,Object.values(r.participants||{}).map(u=>u.name).join(' / '),r.payerName||'',r.memo||'',r.receiptSummary?.count||0,r.fxRate||'',r.fxRateDate||'',r.estimatedKrw||'',r.createdByName||'',c.croatia,c.korea,r.updatedByName||'',u.croatia,u.korea]})];
  ...
  function memberRole(u){return [u.tripRole,u.leader?'조장':'',u.presenter?'발표':''].filter(Boolean).join(' · ')}
  function exportSettlementExcel(){
    if(!Integration.canManageFinance()||!storageReady){ledgerNotice='공유 장부 연결 후 Excel 파일을 만들 수 있습니다.';renderRecords();return}if(!window.XlsxLite){ledgerNotice='Excel 내보내기 모듈을 불러오지 못했습니다.';renderRecords();return}
    const entries=activeEntriesSorted(),detail=[],memberAgg=new Map(all().map(u=>[u.memberId,{eur:0,krw:0,est:0,count:0}]));
    for(const [id,r] of entries){const amount=r.currency!=='KRW'?r.amountMinor/100:r.amountMinor, est=Number.isFinite(r.estimatedKrw)?r.estimatedKrw:(r.currency==='KRW'?r.amountMinor:0), n=Math.max(1,Number(r.participantCount)||1);for(const [mid,p] of Object.entries(r.participants||{})){const m=TEAM_MEMBERS.find(x=>x.memberId===mid)||p,eur=r.currency===FOREIGN?amount/n:0,krw=r.currency==='KRW'?amount/n:0,estShare=est/n;detail.push({mid,name:p.name||m?.name||'',group:p.group??m?.group??0,date:usageDateOf(r),merchant:r.merchant||'',title:r.title||'',currency:r.currency,amount,n,eur,krw,estShare,estTotal:est,payer:r.payerName||'',id});const a=memberAgg.get(mid)||{eur:0,krw:0,est:0,count:0};a.eur+=eur;a.krw+=krw;a.est+=estShare;a.count++;memberAgg.set(mid,a)}}
    const expenseRows=[['사용일','사용시각','사용처','사용목적','분류','통화','금액','참석인원','참석자','결제자','적용환율','환율기준일','원화환산추정','영수증','등록자','저장일시(현지)','저장일시(한국)','수정자','수정일시(현지)','수정일시(한국)','기록ID']];
    for(const [id,r] of entries){const c=recordCreated(r),u=recordUpdated(r);expenseRows.push([usageDateOf(r),r.time||'',r.merchant||'',r.title||'',r.category||'',r.currency,r.currency!=='KRW'?r.amountMinor/100:r.amountMinor,r.participantCount,Object.values(r.participants||{}).map(x=>x.name).join(', '),r.payerName||'',r.fxRate||'',r.fxRateDate||'',r.estimatedKrw||'',r.receiptSummary?.count||0,r.createdByName||'',c.croatia,c.korea,r.updatedByName||'',u.croatia,u.korea,id])}
    const detailRows=[['이름','조','사용일','사용처','사용목적','통화','원금액','참석인원','EUR 분담액','KRW 분담액','원화환산 추정 분담액','원화환산 추정 총액','결제자','기록ID']];
    detail.forEach((d,i)=>{const row=i+2;detailRows.push([d.name,d.group?d.group+'조':'교수',d.date,d.merchant,d.title,d.currency,d.amount,d.n,{f:`IF(F${row}="EUR",G${row}/H${row},0)`,v:d.eur,style:'money2'},{f:`IF(F${row}="KRW",G${row}/H${row},0)`,v:d.krw,style:'integer'},{f:`IF(L${row}>0,L${row}/H${row},IF(F${row}="KRW",G${row}/H${row},0))`,v:d.estShare,style:'integer'},d.estTotal,d.payer,d.id])});
    const summaryRows=[['이름','조','연수 역할','참여 건수','EUR 분담 합계','KRW 분담 합계','원화환산 추정 합계']];
    all().forEach((u,i)=>{const a=memberAgg.get(u.memberId)||{eur:0,krw:0,est:0,count:0},row=i+2;summaryRows.push([u.name,u.group?u.group+'조':'교수',memberRole(u),{f:`COUNTIF('참여자별상세'!$A:$A,A${row})`,v:a.count,style:'integer'},{f:`SUMIF('참여자별상세'!$A:$A,A${row},'참여자별상세'!$I:$I)`,v:a.eur,style:'money2'},{f:`SUMIF('참여자별상세'!$A:$A,A${row},'참여자별상세'!$J:$J)`,v:a.krw,style:'integer'},{f:`SUMIF('참여자별상세'!$A:$A,A${row},'참여자별상세'!$K:$K)`,v:a.est,style:'integer'}])});
    const rosterRows=[['이름','조','연수 역할','기관','기관 직위','전화번호']];all().forEach(u=>rosterRows.push([u.name,u.group?u.group+'조':'교수',memberRole(u),u.org||'',u.title||'',u.phone||'']));
    const sheets=[{name:'경비내역',rows:expenseRows,widths:[12,10,24,28,14,10,14,10,36,16,14,12,16,10,12,20,20,12,20,20,28]},{name:'참여자별상세',rows:detailRows,widths:[12,8,12,22,26,9,14,10,14,14,18,18,14,28]},{name:'참여자별정산',rows:summaryRows,widths:[12,8,20,10,16,16,18]},{name:'참여자명단',rows:rosterRows,widths:[12,8,20,24,18,16]}];
  ...
  async function edit(id){
    try{requireStaff();if(!storageReady)throw Error('공유 저장 연결을 확인해 주세요.');if(formHasValues()&&!confirm('작성 중인 입력 대신 이 공유 기록을 불러올까요?'))return;const r=await Integration.api('expenses/'+TRIP_CODE+'/'+id,{headers:{'X-Firebase-ETag':'true'}});if(!Integration.canManageFinance()||!r.data)return;
      ...
      for(const [el,k] of [['exDate','date'],['exTime','time'],['exMerchant','merchant'],['exTitle','title'],['exCategory','category'],['exCurrency','currency'],['exPayer','payerName'],['exMemo','memo']])document.getElementById(el).value=d[k]||'';document.getElementById('exAmount').value=d.currency!=='KRW'?(d.amountMinor/100).toFixed(2):String(d.amountMinor);selected=new Set(Object.keys(d.participants||{}));mode=d.selectionMode;fxQuote=d.fxRate?{rate:d.fxRate,asOf:d.fxRateDate||d.date,exact:d.fxExact!==false,basis:d.fxBasis||'저장 당시 하나은행 매매기준율',source:d.fxSource||'하나은행'}:null;renderSelection();renderFxEstimate();document.getElementById('expenseFormTitle').textContent='공유 기록 수정';scheduleFxEstimate();setExpenseTab('entry');status('날짜·사용처·금액·참여자를 확인한 뒤 저장해 주세요.');document.getElementById('expenseFormTitle').scrollIntoView({block:'start',behavior:'smooth'});
    }catch(e){status(e.message,'error')}
  }
  function validate(){
    const x=snapshot(),merchant=x.merchant.trim(),title=x.title.trim();if(!merchant)throw Error('사용처·상호를 입력해 주세요.');if(!title)throw Error('사용 목적을 입력해 주세요.');if(!/^\d{4}-\d{2}-\d{2}$/.test(x.date))throw Error('사용일을 선택해 주세요.');let raw=x.amount.trim();if(x.currency!=='KRW')raw=raw.replace(',','.');const valid=x.currency!=='KRW'?/^\d+(\.\d{1,2})?$/.test(raw):/^\d+$/.test(raw);if(!valid)throw Error(x.currency!=='KRW'?'현지통화 금액은 소수점 두 자리까지 입력해 주세요.':'원화 금액은 원 단위 정수로 입력해 주세요.');const amountMinor=Math.round(Number(raw)*(x.currency!=='KRW'?100:1));if(!Number.isSafeInteger(amountMinor)||amountMinor<=0||amountMinor>1000000000000)throw Error('금액을 다시 확인해 주세요.');if(!selected.size)throw Error('참여자를 한 명 이상 선택해 주세요.');const participants={};all().filter(u=>selected.has(keyOf(u))).forEach(u=>participants[keyOf(u)]={name:u.name,group:u.group});if(Object.keys(participants).length!==selected.size)throw Error('참여자 목록을 다시 선택해 주세요.');const amountMajor=x.currency!=='KRW'?amountMinor/100:amountMinor;const fx=x.currency==='EUR'&&fxQuote?{fxRate:+fxQuote.rate,fxRateDate:fxQuote.asOf||x.date,fxExact:!!fxQuote.exact,fxBasis:fxQuote.basis||'하나은행 매매기준율',fxSource:fxQuote.source||'하나은행',estimatedKrw:ExpenseFx.krw(amountMajor,fxQuote.rate)}:{estimatedKrw:x.currency==='KRW'?amountMinor:null};return {date:x.date,time:x.time||'',merchant,title,category:x.category,currency:x.currency,amountMinor,payerName:x.payer.trim(),memo:x.memo.trim(),selectionMode:mode,participants,participantCount:selected.size,receiptExtracted:!!ocrResult,receiptOcrProvider:ocrResult?.provider||'',receiptOcrConfidence:ocrResult?.confidence||0,...fx};
  }
  async function save(){
  ...
  document.addEventListener('click',e=>{
    ...
    const del=e.target.closest('[data-delete-expense]');if(del&&Integration.canManageFinance()){deleteExpense(del.dataset.deleteExpense);return}
