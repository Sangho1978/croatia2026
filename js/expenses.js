...
(function(){
  ...
  let expenseTab='entry',ledgerDay='all',ledgerNotice='';
      ...
      <section class="expense-pane expense-pane-entry" data-expense-pane="entry">
        <div class="finance-panel"><h3 id="expenseFormTitle">공동경비 등록</h3><form id="expenseForm" class="finance-form">
          <div class="receipt-first">
            ...
            <div id="receiptReadStatus" class="ocr-status">영수증을 올리면 날짜·사용처·금액을 자동으로 읽고 아래 입력칸에 채웁니다.</div>
          ...
          <div class="finance-row"><label class="field">통화<select id="exCurrency"><option value="EUR">EUR · 유로</option><option value="KRW">KRW · 원</option></select></label><label class="field">전체 금액<input id="exAmount" inputmode="decimal" placeholder="0.00" required /></label></div>
          <label class="field">결제자<input id="exPayer" maxlength="60" placeholder="실제 결제한 사람 또는 공용카드" /></label>
          <h4 style="margin:5px 0">② 참여자</h4><div class="expense-mode" role="group" aria-label="경비 참여 범위"><button type="button" data-expense-mode="all">전체 · 28명</button><button type="button" data-expense-mode="selected">개별 선택</button></div>
  ...
  function renderBase(){
    ...
    form.addEventListener('input',e=>{pendingId=pendingId||newId();if(e.target.matches('input,select,textarea'))e.target.classList.remove('ocr-filled')});
    ...
    document.getElementById('sameDayAdd').onclick=()=>{const d=ledgerDay!=='all'?ledgerDay:nowLocalDate();newForm();document.getElementById('exDate').value=d;setExpenseTab('entry')};
  ...
  function resetOcr(){ocrResult=null;const r=document.getElementById('ocrReview');if(r){r.hidden=true;r.innerHTML=''}const s=document.getElementById('receiptReadStatus');if(s){s.className='ocr-status';s.textContent='영수증을 올리면 날짜·사용처·금액을 자동으로 읽고 아래 입력칸에 채웁니다.'}}
  function newForm(){
    ...
    document.getElementById('expenseFormTitle').textContent='공동경비 등록';renderSelection();status('');
  }
  function snapshot(){return {date:document.getElementById('exDate').value,merchant:document.getElementById('exMerchant').value,title:document.getElementById('exTitle').value,category:document.getElementById('exCategory').value,currency:document.getElementById('exCurrency').value,amount:document.getElementById('exAmount').value,payer:document.getElementById('exPayer').value,memo:document.getElementById('exMemo').value,mode,selected:[...selected],pendingId,editId,editEtag,original,receiptChanged,receiptBatch,ocrResult}}
  ...
  function restoreDraft(){
    try{const raw=localStorage.getItem(draftKey())||localStorage.getItem(oldDraftKey())||'null',d=JSON.parse(raw);if(!d)return;
      for(const [id,k] of [['exDate','date'],['exMerchant','merchant'],['exTitle','title'],['exCategory','category'],['exCurrency','currency'],['exAmount','amount'],['exPayer','payer'],['exMemo','memo']])if(document.getElementById(id))document.getElementById(id).value=d[k]||'';
      selected=new Set((d.selected||[]).filter(id=>TEAM_MEMBERS.some(u=>u.memberId===id)));mode=d.mode==='all'?'all':'selected';if(mode==='all')selected=new Set(all().map(keyOf));pendingId=d.pendingId||newId();editId=d.editId||null;editEtag=d.editEtag||null;original=d.original||null;ocrResult=d.ocrResult||null;renderSelection();
  ...
  function renderRecords(){
    if(!document.getElementById('expenseRecords'))return;const items=visibleItems(),sums={EUR:0,KRW:0},daily={};items.forEach(([,r])=>{if(r.currency in sums&&Number.isFinite(r.amountMinor)){sums[r.currency]+=r.amountMinor;(daily[r.date]??={EUR:0,KRW:0,n:0})[r.currency]+=r.amountMinor;daily[r.date].n++}});renderDayChips();
    ...
    document.getElementById('expenseRecords').innerHTML=Object.keys(grouped).length?Object.entries(grouped).sort(([a],[b])=>b.localeCompare(a)).map(([date,rows])=>{
      const d=daily[date]||{EUR:0,KRW:0,n:0},tot=[d.EUR?money(d.EUR,'EUR'):'',d.KRW?money(d.KRW,'KRW'):''].filter(Boolean).join(' / ');
  ...
  function renderReceiptEditor(){
    ...
    box.innerHTML=receiptItems.map((r,i)=>`<figure class="receipt-thumb"><img src="${escape(r.dataUrl)}" alt="영수증 미리보기 ${i+1}"><figcaption>${Math.ceil(r.bytes/1024)} KB <button type="button" data-ocr-receipt="${i}">자동 읽기</button> <button type="button" data-remove-receipt="${i}" aria-label="사진 ${i+1} 제거">제거</button></figcaption></figure>`).join('')||'<p class="quiet">선택한 영수증이 없습니다.</p>';
  }
  function renderOcrReview(){
    const box=document.getElementById('ocrReview');if(!box)return;if(!ocrResult){box.hidden=true;box.innerHTML='';return}box.hidden=false;box.innerHTML=`<span>날짜<b>${escape(ocrResult.date||'확인 필요')}</b></span><span>사용처<b>${escape(ocrResult.merchant||'확인 필요')}</b></span><span>금액<b>${ocrResult.amount!=null?escape((ocrResult.currency==='EUR'?'€':'₩')+ocrResult.amount):'확인 필요'}</b></span><span>OCR 품질<b>${ocrResult.confidence?ocrResult.confidence+'%':'확인 필요'}</b></span>`;
  }
  ...
  function applyOcr(r){ocrResult=r;renderOcrReview();if(r.date)markAuto('exDate',r.date);if(r.merchant)markAuto('exMerchant',r.merchant);if(r.amount!=null)markAuto('exAmount',r.currency==='EUR'?Number(r.amount).toFixed(2):String(Math.round(r.amount)));if(r.currency)markAuto('exCurrency',r.currency);if(r.category)markAuto('exCategory',r.category);if(r.purpose&&!document.getElementById('exTitle').value.trim())markAuto('exTitle',r.purpose);saveDraft(false)}
  async function runOcr(index=0){
    if(ocrBusy||!receiptItems[index])return;const st=document.getElementById('receiptReadStatus');ocrBusy=true;const save=document.getElementById('expenseSave');if(save)save.disabled=true;try{st.className='ocr-status working';st.textContent='영수증 자동인식 준비 중…';const r=await ReceiptOCR.recognize(receiptItems[index].dataUrl,p=>{st.textContent='영수증 읽는 중… '+p+'%'});applyOcr(r);const count=[r.date,r.merchant,r.amount!=null].filter(Boolean).length;st.className='ocr-status '+(count>=2?'success':'error');st.textContent=count>=2?'✓ 자동입력 완료 · 노란 입력칸을 확인하고 틀리면 바로 수정하세요.':'자동인식이 부족합니다. 사진을 보면서 날짜·사용처·금액을 직접 입력해 주세요.';}
    catch(e){st.className='ocr-status error';st.textContent='자동인식 실패 · '+e.message+' 직접 입력은 계속 가능합니다.';}
    finally{ocrBusy=false;if(save)save.disabled=!storageReady||receiptBusy;}
  }
  ...
  document.addEventListener('click',e=>{
    ...
    const ocr=e.target.closest('[data-ocr-receipt]');if(ocr)runOcr(+ocr.dataset.ocrReceipt);
